...
//		System.out.println(values.length);
		...
		for (int j = 1; j < shuffled.length; j = j + 2) {
			shuffled[j] = values[index];
			index++;
		}
		for (int k = 1; k < shuffled.length; k++) {
			values[k] = shuffled[k];
		}
	...
	public static void selectionShuffle(int[] values) {
		...
		for (int j = 0; j< shuffled.length; j++) {
			shuffled[j] = VALUE_COUNT;
		}
		boolean asdf = true; ;
		for (int i = 0; i < shuffled.length; i++) {
			while (shuffled[i] == VALUE_COUNT) {
				int j = (int)(Math.random() * (VALUE_COUNT));
